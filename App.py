...
def processar_prever(df):
    encoder_sexo = encoder.fit_transform(df[['sexo']])
    encoded_df = pd.DataFrame(encoder_sexo, columns=encoder.get_feature_names_out(['sexo']))
    dados = pd.concat([df, encoded_df], axis=1).drop(columns=['sexo'], axis=1)
...
if up_file is not None:
        ...
        st.write('Visualização dos resultados (15 primeiros registros)')
        st.write(df.head(15))
